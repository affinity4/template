...
use Affinity4\Template\Syntax\Dust;

class DustSyntaxTest extends TestCase
{
    private $vfs;
    private $template;
    private $view_path;
    ...
    public function setUp()
    {
        ...
        $this->template = new Engine(new Dust());
        $this->view_path = 'tests/views/dust';
    }

    public function testCompile()
    {
        $this->template->setStream('{! This is a comment !}');
        $this->template->compile($this->template->getStream());

        $this->assertEquals('<?php // This is a comment ?>', $this->template->getStream());
    }
    ...
    public function testComments()
    {
        ...
        ob_start();
        $this->template->render($this->view_path . '/comments.php');
        $output = ob_get_clean();
        $this->assertEquals($expected, $output);
    }

    public function testPartialsAndBlocks()
    {
        $expected = <<<PARTIALS
<!doctype>
<html>
</html>
    <title>{% block (.*) %}Default Title{% endblock %}</title>
</head>
<body>
    {% block content %}
        Default Contents
